...
export default function Salary() {
  ...
  const totalSalary = report?.reduce((s, r) => s + r.finalSalary, 0) || 0;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="border rounded-lg overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && report?.length === 0 && <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">No data for this period.</TableCell></TableRow>}
          </TableBody>
          {(report?.length ?? 0) > 0 && (
            <tfoot>
              <tr className="bg-muted/50 font-bold border-t">
                <td colSpan={8} className="px-4 py-3 text-sm">Total Salary Payable</td>
                <td className="px-4 py-3 text-primary font-bold">&#8377;{totalSalary.toFixed(2)}</td>
              </tr>
            </tfoot>
          )}
        </Table>
      ...
      <p className="text-xs text-muted-foreground">Salary formula: (Present days × daily rate) + (Half days × 0.5 × daily rate)</p>
    </div>
  );
}
